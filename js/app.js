...
var mainView = app
    .views
    .create('.view-main');
var categoriesView = app
    .views
    .create('#view-categories');
var discoverView = app
    .views
    .create('#view-discover');
var searchView = app
    .views
    .create('#view-search');
var pagesView = app
        ...
        on: {
            search(sb, query) {
                if (query == '') {
                    ...
                    // Emulate 0.5s loading for the demo You can do an Ajax request here
                    setTimeout(function () {
...
$$('.ptr-content').on('ptr:refresh', function (e) {
    // Emulate 1s loading You can do an Ajax request here to retrieve your posts
    // from a database
    setTimeout(function () {
        var html = '<div class="title-container"><span class="title-date">Tuesday 19 March</span><h1' +
                '>Just Now</h1></div><a href="/single/"><div class="card"><img class="card-image"' +
                ' src="img/thumb-14.jpg" alt=""><div class="card-infos"><h2 class="card-title">Ho' +
                'w to Get Your First Tattoo Right</h2><div class="card-bottom"><div class="card-a' +
                'uthor"><img class="card-author-image" src="img/authors/author-1.jpg" alt=""><div' +
                '>Camille Aline</div></div><div class="card-comments"><i class="icon ion-ios-text' +
                '"></i>3</div></div></div></div></a>';
...
$$('.infinite-scroll-content').on('infinite', function () {
    ...
    // Emulate 1s loading (You can do an Ajax request here to retrieve your next
    // posts)
    setTimeout(function () {
        allowInfinite = true;

        // Generate new items HTML for the demo
        var html = '<li><a href="/single/"><div class="item-content"><div class="item-media"><img sr' +
                'c="img/thumb-15.jpg" width="44"/></div><div class="item-inner"><div class="item-' +
                'subtitle">Fashion</div><div class="item-title">Archery at the 2024 Olympic Games' +
                '</div><div class="item-subtitle bottom-subtitle"><img src="img/authors/author-3.' +
                'jpg">Jess Roxana</div></div></div></a></li><li><a href="/single/"><div class="it' +
                'em-content"><div class="item-media"><img src="img/thumb-16.jpg" width="44"/></di' +
                'v><div class="item-inner"><div class="item-subtitle">Fashion</div><div class="it' +
                'em-title">Most Beautiful Beach of the Costa Brava</div><div class="item-subtitle' +
                ' bottom-subtitle"><img src="img/authors/author-2.jpg">Zorka Ivka</div></div></di' +
                'v></a></li>';

        // Append new items
        $$('#infinite-content').append(html);

        // Nothing more to load, detach infinite scroll events to prevent unnecessary
        // loadings
        app
            .infiniteScroll
            .destroy('.infinite-scroll-content');
...
$$(document).on('page:init', '.page[data-name="pull-to-refresh"]', function (e) {
    $$('#pages-ptr')
        .on('ptr:refresh', function (e) {
            // Emulate 1s loading You can do an Ajax request here to retrieve your posts
            // from a database
            setTimeout(function () {
                var html = '<li><a href="/single/"><div class="item-content"><div class="item-media"><img sr' +
                        'c="img/thumb-25.jpg" width="44"/></div><div class="item-inner"><div class="item-' +
                        'subtitle">Fashion</div><div class="item-title">The Best Diet for a Flatter Belly' +
                        '</div><div class="item-subtitle bottom-subtitle"><i class="icon ion-md-time"></i' +
                        '>2 hours ago</div></div></div></a></li>';
...
$$(document).on('page:init', '.page[data-name="infinite-scroll"]', function (e) {
    ...
    $$('#pages-infinite-scroll').on('infinite', function () {
        ...
        setTimeout(function () {
            ...
            // Generate new items HTML for the demo
            var html = '<li><a href="/single/"><div class="item-content"><div class="item-media"><img sr' +
                    'c="img/thumb-26.jpg" width="44"/></div><div class="item-inner"><div class="item-' +
                    'subtitle">Fashion</div><div class="item-title">The Best Diet for a Flatter Belly' +
                    '</div><div class="item-subtitle bottom-subtitle"><i class="icon ion-md-time"></i' +
                    '>2 hours ago</div></div></div></a></li>';
            ...
            [
                {
                    ...
                    bold: true,
                    onClick: function () {
                        window.open('http://twitter.com/share?text=Welcome%20To%20Yui&url=http://themeforest.net&hash' +
...
// Attach the Share Dialog event to all elements that have the share-actions
// class
...
var myNotification = app
    .notification
    .create({icon: '<i class="icon ion-ios-notifications"></i>', title: 'Yui Template', subtitle: 'This is a mobile notification', text: 'Click (x) button to close me', closeButton: true});

// Attach the notification event to all elements that have the open-notification
// class
...
// var list_col = $$(".list-col"); $$(list_col).each(function(index){
// if(($$(this).children(".number-col").length) > 1 &&
// ($$(this).children(".number-col").length) <=2){
